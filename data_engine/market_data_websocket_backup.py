...
try:
    ...
    from SmartApi.smartApiWebsocket import SmartWebSocket  # V1 - classic WebSocket
except ImportError:
    raise ImportError(
...
class WebSocketFeedHandler:
    ...
    def __init__(self, auth_token: str, feed_token: str, client_code: str):
        """
        Initialize WebSocket handler for Angel One.
        
        Args:
            auth_token: JWT token from authentication (not used by V1 WebSocket)
            feed_token: Feed token for WebSocket connection
            client_code: Client code for WebSocket connection
        """
        self.auth_token = auth_token
        self.feed_token = feed_token
        self.client_code = client_code
        
        # Initialize SmartWebSocket V1
        ...
        # Register callbacks
        self.ws.on_open = self._on_open_callback
        self.ws.on_message = self._on_message_callback
        self.ws.on_close = self._on_close_callback
        self.ws.on_error = self._on_error_callback
        ...
        self._is_open = False  # Track if WebSocket has opened
    
    def connect(self) -> bool:
        """
        Establish WebSocket connection to Angel One using SmartWebSocket V1.
        
        Returns:
            True if connection successful, False otherwise
        """
        try:
            print("\n[WebSocket] Initializing SmartWebSocket V1 connection...")
            print(f"   Client Code: {self.client_code}")
            print(f"   Feed Token: {self.feed_token[:20]}...")
            logger.info("Attempting WebSocket connection to Angel One (V1)...")
            
            print("[WebSocket] SmartWebSocket object created")
            print("[WebSocket] Event handlers registered")
            
            # Connect in a separate thread (non-blocking)
            print("[WebSocket] Starting connection in background thread...")
            ...
            print("[WebSocket] Waiting for connection...")
            max_wait = 5.0
            wait_start = time.time()
            while not self._is_open and (time.time() - wait_start) < max_wait:
                time.sleep(0.1)
            
            if self._is_open:
                print("[WebSocket] ✓ Socket is now OPEN and ready")
            else:
                print(f"[WebSocket] ⚠ Timeout waiting for open (waited {time.time() - wait_start:.1f}s)")
            
            self._running = True
            self._reconnect_attempts = 0
            
            print("[WebSocket] Returning from connect()")
    ...
    def subscribe(self, symbol: str, mode: Mode = Mode.LTP) -> bool:
        """
        Subscribe to price updates for a symbol using SmartWebSocket V1.
        
        Args:
            symbol: Trading symbol
            mode: Subscription mode (LTP, QUOTE, FULL) - V1 uses "mw" (market watch)
        ...
        try:
            ...
            if self.ws:
                ...
                print(f"      Token: {token}")
                print(f"      WebSocket status: open={self._is_open}")
                
                try:
                    # V1 API: subscribe(task, token)
                    # task="mw" is for market watch (price updates)
                    print(f"   📤 Calling ws.subscribe('mw', '{token}')")
                    self.ws.subscribe("mw", token)
                    self._subscribed_tokens[token] = symbol
                    logger.info(f"Subscribed to {symbol} (token: {token})")
                    print(f"   ✓ Subscription sent to WebSocket API")
                    return True
                except Exception as sub_error:
                    print(f"   ✗ Subscription failed: {sub_error}")
                    logger.error(f"Subscription error: {sub_error}")
                    return False
            else:
    ...
    def _on_open_callback(self, *args):
        """Callback wrapper for on_open events from SmartWebSocket V1."""
        self._on_open()
    
    def _on_message_callback(self, *args):
        """Callback wrapper for on_message events from SmartWebSocket V1."""
        # V1 passes message as a string or dict, need to handle both
        if args and len(args) > 0:
            message = args[0]
            self._on_message(None, message)
    
    def _on_close_callback(self, *args):
        """Callback wrapper for on_close events from SmartWebSocket V1."""
        self._on_close()
    
    def _on_error_callback(self, *args):
        """Callback wrapper for on_error events from SmartWebSocket V1."""
    ...
    def _on_message(self, ws, message: str):
        ...
        if not hasattr(self, '_msg_debug_count'):
            ...
            print("\n[DEBUG] First _on_message callback received!")
        self._msg_debug_count += 1
        
        try:
            msg_preview = str(message)[:150] if len(str(message)) > 150 else str(message)
            print(f"\n📨 [WebSocket Message #{self._msg_debug_count}] {msg_preview}..." if len(str(message)) > 150 else f"\n📨 [WebSocket Message #{self._msg_debug_count}] {message}")
            ...
            print(f"\n⚠ [Non-JSON Message] {str(message)[:100]}... (Error: {e})")
    ...
    def get_price_updates(self) -> List[Dict]:
        ...
        try:
            while True:
                data = self._price_queue.get_nowait()
                updates.append(data)
                if len(updates) == 1:
                    print(f"      [get_price_updates] Got update from queue: {str(data)[:100]}...")
        except queue.Empty:
            pass
        
        if updates and len(updates) > 0:
            print(f"      [get_price_updates] Returning {len(updates)} update(s)")
        return updates
...
class MarketDataEngine:
    ...
    def initialize(self) -> bool:
        ...
        self.ws_handler = WebSocketFeedHandler(auth_token, feed_token, Settings.CLIENT_ID)
    ...
    def get_price_data(self, symbol: str) -> Optional[PriceData]:
        ...
        """
        price_data = self.cache.get(symbol)
        if price_data:
            print(f"   ✓ Got price for {symbol}: LTP={price_data.ltp}, Bid={price_data.bid}, Ask={price_data.ask}")
        else:
            print(f"   ⚠ No price data for {symbol} yet")
        return price_data
    ...
    def _update_loop(self) -> None:
        ...
        """
        print("\n[Background Thread] Update loop started...")
        ...
        while self._running:
            try:
                ...
                if cycle_count % 10 == 0:  # Print every 10 cycles
                    print(f"[Background] Cycle {cycle_count}: Checking for WebSocket updates...")
                
                # Process WebSocket updates
                if self.ws_handler:
                    updates = self.ws_handler.get_price_updates()
                    if updates:
                        print(f"[Background] Got {len(updates)} price update(s) from WebSocket")
                        for update in updates:
                            self._process_price_update(update)
                    else:
                        if cycle_count % 20 == 0:
                            print(f"[Background] No updates from WebSocket yet")
                else:
                    print(f"[Background] WebSocket handler not available")
                
                # Periodically fetch via REST API
                if cycle_count % int(rest_fetch_interval / Settings.MARKET_DATA_UPDATE_INTERVAL) == 0:
                    print(f"[Background] Attempting periodic REST API fetch...")
    ...
    def _process_price_update(self, update: Dict) -> None:
        ...
        """
        try:
            print(f"         [_process_price_update] Processing: {update}")
            # Extract symbol and price data from update
            # This depends on SmartAPI's exact response format
            symbol = update.get('symbol')
            if not symbol:
                print(f"         [_process_price_update] No symbol in update")
                return
